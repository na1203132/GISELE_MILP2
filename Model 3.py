...
def Radiality_rule(model,node):
   return summation(model.x)==len(model.N)-24
model.Radiality = Constraint(model.N,rule=Radiality_rule)

def Power_flow_conservation_rule(model,node):
    return (sum(model.P[j,node]for j in model.NodesIn[node])-sum(model.P[node,j] for j in model.NodesOut[node])) ==  model.Psub[node] - model.PPS[node] * model.PS[node]
